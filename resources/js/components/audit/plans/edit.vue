            ...
            <div class="col-12 grid-margin">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="form-sample" @submit.prevent="editPlan">
...
export default {
    ...
    methods: {
        ...
        async planShow()
        {
            ...
            const res   = await this.callApi('post' ,   '/plans/'+this.$route.params.id+'/show'    ,   null)

            this.plan.nom                   =   res.data.nom
            this.plan.date_debut            =   res.data.date_debut
            this.plan.date_fin              =   res.data.date_fin
            this.plan.id_type_audit         =   res.data.id_type_audit
            this.plan.id_societee           =   res.data.id_societee
        ...
        {   
            ...
            const res   = await this.callApi('post' ,   '/plans/'+this.$route.params.id+'/update'    ,   formData)

            if(res.status===200){
                this.feedbackSuccess("Plan has been modified !"     ,   "a new plan has been added")
                this.$router.push('/plans');
			}
            
            else{
                this.showErrors("Plan has not been modified !", res.data.errors)
			}
        },
