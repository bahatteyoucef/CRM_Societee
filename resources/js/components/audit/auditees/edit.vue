            ...
            <div class="col-12 grid-margin">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="form-sample" @submit.prevent="editAuditee">
...
export default {
    ...
    methods: {
        ...
        async auditeeShow()
        {
            ...
            const res   = await this.callApi('post' ,   '/auditees/'+this.$route.params.id+'/show'    ,   null)

            this.auditee.nom                    =   res.data.nom
            this.auditee.prenom                 =   res.data.prenom
            this.auditee.id_fonction            =   res.data.id_fonction
            this.auditee.id_structure           =   res.data.id_structure
            this.auditee.id_societee            =   res.data.id_societee
        },
        ...
        {   
            ...
            const res   = await this.callApi('post' ,   '/auditees/'+this.$route.params.id+'/update'    ,   formData)

            if(res.status===200){
                this.feedbackSuccess("Auditee has been modified !"     ,   "an auditee has been modified")
                this.$router.push('/auditees');
			}
            
            else{
                this.showErrors("Auditee has not been modified !", res.data.errors)
			}
        },
